...
namespace Utils
{
    bool IsAudioFileSupported(const std::string& file_path)
    {
    ...
    bool GetAudioInfo(const std::string& file_path, int& sample_rate, int& channels, int& duration_ms)
    {
        // 这里需要实现音频文件信息读取
        // 可以使用现有的音频库或者系统调用
    ...
    bool ConvertAudioFormat(const std::string& input_path, const std::string& output_path,
